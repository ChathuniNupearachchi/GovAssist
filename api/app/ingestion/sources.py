...
SOURCES: list[Source] = [
    ...
    Source(
        ...
        note="Scanned, no text layer. Ingested (`app.ingestion.phase9_child_"
        "deletion`) for service #6's own implementation — re-attempted through "
        "the full free extraction chain, same garbled result as the original "
        "research pass (Tesseract quality check apparently still passed on "
        "largely non-English text; Gemini vision not triggered again). English "
        "portions legible (confirms 'APPLICATION FOR DELETION OF CHILDREN', "
        "photo/attestation requirements, and a legible 'form 35C' reference); "
        "Sinhala/Tamil portions garbled. Rs.250 figure still present, still not "
        "legible with confidence — NOT encoded as a FeeRule (the LKR 1,200 "
        "deletion fee is independently confirmed by id=10 and instructions_"
        "english_td.pdf, both clean sources, so this document's own content "
        "isn't needed for the fee — only cited as the downloadable form "
        "itself). id=24 labels this file's download link 'Form C' for "
        "'Children Deletion', a separate line item from 'Form O' "
